...
SingletrackODE::SingletrackODE(double deltaT, tyreModel tyre_model) : dt(deltaT),
...
void SingletrackODE::setInitialState(double r0, double beta0, double x0, double y0, double psi0)
{
    // Initial state values
    state[0] = r0;
    state[1] = beta0;
...
void SingletrackODE::setVehicleParams(double m, double a, double b, double Cf, double Cr, double mu, double Iz)
{
...
void SingletrackODE::setReferenceCommands(double velocity, double steer)
{
    Vx    = velocity;
    delta = steer;
}

void SingletrackODE::integrate()
{
    ...
    stepper.do_step(std::bind(&SingletrackODE::vehicle_ode, this, _1, _2, _3), state, t, dt);

    // Update time and steering
    t += dt;
}

void SingletrackODE::vehicle_ode(const state_type &state, state_type &dstate, double t)
{
