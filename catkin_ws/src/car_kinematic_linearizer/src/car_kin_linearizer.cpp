...
CarKinLinearizer::CarKinLinearizer(ros::NodeHandle n)
{    
    ROS_ASSERT_MSG(n.hasParam("/car_kinematic_linearizer/wheel_distance"), "YAML param does not exists.");
    node_handle_.getParam("/car_kinematic_linearizer/wheel_distance", model_params_.L);
    if (model_params_.L < 0.0) throw new ros::InvalidParameterException("Wheel distance cannot be below 0.0");

    ROS_ASSERT_MSG(n.hasParam("/car_kinematic_linearizer/epsilon"), "YAML param does not exists.");
    node_handle_.getParam("/car_kinematic_linearizer/epsilon", model_params_.epsilon);
    if (model_params_.epsilon < 0.0) throw new ros::InvalidParameterException("Distance from point P and COG cannot be below 0.0");
    
    node_handle_ = n;
}

void CarKinLinearizer::prepare()
{
    sub_input_velocity_ = node_handle_.subscribe("/virtual_velocities", 1000, &CarKinLinearizer::inputVelocityCallback, this);
    sub_odometry_ = node_handle_.subscribe("/vesc/odom", 1000, &CarKinLinearizer::odometryCallback, this);
...
void CarKinLinearizer::runPeriodically()
{
    linearize();
    sendCommand();
}

void CarKinLinearizer::shutDown()
{

}

void CarKinLinearizer::linearize()
{
...
void CarKinLinearizer::sendCommand() const
{
...
void CarKinLinearizer::inputVelocityCallback(const std_msgs::Float64MultiArray::ConstPtr& input_vel_msg)
{
...
void CarKinLinearizer::odometryCallback(const nav_msgs::Odometry::ConstPtr& odom_msg)
{
    double roll {0.0};
    double pitch {0.0};
    double yaw {0.0};
